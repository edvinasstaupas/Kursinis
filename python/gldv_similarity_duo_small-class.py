...
dataset_directory = "train_split_20"
checkpoint_path = (
    "training_1/checkpoints/resnet-similarity-gldv-small-class/cp-{epoch:04d}.ckpt"
)
...
def pair_values(map_of_lists):
    ...
    for key, values in map_of_lists.items():
        for i in range(0, len(values), 2):
            paired_values.append((values[i], values[i + 1], key))
...
def generator_with_cache(ds, cache):
    while True:
        ...
        for item1, item2, label in paired_ds:
...
filename = "matrix/resnet-similarity-gldv-small-class-e100/matrix.txt"
filename2 = "matrix/resnet-similarity-gldv-small-class-e100/matrix.npy"
...
filename3 = "matrix/resnet-similarity-gldv-small-class-e100/pred.txt"
